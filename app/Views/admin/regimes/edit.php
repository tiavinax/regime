...
<head>
    ...
    <title>Modifier un régime - Admin NutriGoal</title>
...
<body>
    <div class="admin-container">
        ...
        <div class="main-content">
            <div class="top-bar">
                <h1><i class="fas fa-edit"></i> Modifier le régime</h1>
                <div>
            ...
            <div class="form-card">
                <form action="/admin/regimes/update/<?= $regime['id'] ?>" method="POST">
                    <?= csrf_field() ?>

                    <div class="form-group">
                        <label>Nom du régime *</label>
                        <input type="text" name="nom_regime" value="<?= esc($regime['nom_regime']) ?>" required>
                    </div>

                    <div class="form-group">
                        <label>Description *</label>
                        <textarea name="description" rows="4" required><?= esc($regime['description']) ?></textarea>
                    </div>

                    <div class="form-group">
                        <label>Type cible *</label>
                        <select name="type_cible" required>
                            <option value="augmenter" <?= $regime['type_cible'] == 'augmenter' ? 'selected' : '' ?>>⬆️ Augmenter (prise de poids)</option>
                            <option value="reduire" <?= $regime['type_cible'] == 'reduire' ? 'selected' : '' ?>>⬇️ Réduire (perte de poids)</option>
                            <option value="maintenir" <?= $regime['type_cible'] == 'maintenir' ? 'selected' : '' ?>>⚖️ Maintenir (équilibre)</option>
                        </select>
                    </div>

                    <div class="form-group">
                        <label>Apport calorique référence (kcal)</label>
                        <input type="number" name="apport_calorique_reference" value="<?= $regime['apport_calorique_reference'] ?>">
                    </div>

                    <div class="form-group">
                        <label>Prix journalier (€)</label>
                        <input type="number" step="0.01" name="prix_journalier" value="<?= $regime['prix_journalier'] ?>">
                    </div>

                    <div class="form-group">
                        <label>Variation poids/semaine (kg)</label>
                        <input type="number" step="0.1" name="variation_poids_semaine" value="<?= $regime['variation_poids_semaine'] ?>">
                    </div>
